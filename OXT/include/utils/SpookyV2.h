...
public:
  ...
  SpookyHash(uint64 outSeed) { seed = outSeed; }

  uint64 GetSeed() { return seed; }
  //
  // SpookyHash: hash a single message in one call, produce 128-bit output
  //
  void Hash128(const void *message, // message to hash
  ...
  //
  uint64 Hash64(const void *message, // message to hash
                size_t length)       // length of message in bytes
  {
    uint64 hash1 = seed;
    uint64 hash2 = seed;
  ...
  //
  uint32 Hash32(const void *message, // message to hash
                size_t length)       // length of message in bytes
  {
