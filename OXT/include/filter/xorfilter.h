...
template <typename ItemType, typename FingerprintType,
          typename HashFamily = SpookyHash>
class XorFilter {
public:
  size_t size;
  size_t arrayLength;
...
Status XorFilter<ItemType, FingerprintType, HashFamily>::AddAll(
  ...
  while (true) {
    ...
    for (size_t i = start; i < end; i++) {
      uint64_t k = keys[i]; // string can be converted into?
      uint64_t hash = hasher.Hash64(&k, sizeof(ItemType));
...
Status XorFilter<ItemType, FingerprintType, HashFamily>::Contain(
  ...
  uint64_t hash = hasher.Hash64(&key, sizeof(ItemType));
